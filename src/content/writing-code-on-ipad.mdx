...
#### E: Solved ~Local development environment~

Update: The port forwarding just miraculously started working automagically. Probably the solution is to just wait. Or start and restart the environment. Couldn't figure out a pattern yet.

Previous issue: It appears that I'm not able access the localhost, even though there's the port forwarding option. It errors with "Unable to forward localhost:4000. The host may not be available or that remote port may already eb forwarded".
